...
class Train:
    ...
    def __init__(self, data, data_parser, train_ratio=.7):
        ...
        self.train_data, self.test_data = train_test_split(self.data,
                                                           ...
                                                           random_state=0,
                                                           stratify=self.data[['strat']])

        self.train_data_w, self.test_data_w = train_test_split(self.data_w,
                                                               test_size=(1-train_ratio),
                                                               random_state=0,
                                                               stratify=self.data_w[['strat']])

        self.train_data_m, self.test_data_m = train_test_split(self.data_m,
                                                               test_size=(1-train_ratio),
                                                               random_state=0,
                                                               stratify=self.data_m[['strat']])
...
def qini(ts, rs, u_hats, ups, treatment_names=['T=1'], urf_label=['Uplift Random Forest'], urf_colors=['firebrick'],
         urf_colors_bands=['deeppink'], yticks=[0, 0.04]):
